...
import { mobileValidator, usernameValidator } from "./validations.js";
...
export const ControlledForm = () => {
    ...
    const postData=async (userData)=>{
      try{
        let {data}=await axios.post('http://localhost:3000/data',userData)
        fetchData()
        setUsername('');
    ...
    const deleteApiData=async (id)=>{
      try{
        let {data}=await axios.delete('http://localhost:3000/data'+id)
        console.log(data)
        fetchData()
      }
      catch(err){
